...
def test_execute_all_routine_in_specific_flow(dummy_flow_with_two_routines: Flow):
    TEST_SPECIFIC_EVENT = Method("start-Flow1")

    dummy_flow_with_two_routines.execute_event(TEST_SPECIFIC_EVENT)
...
def test_execute_specific_routine(dummy_flow_with_two_routines: Flow):
    TEST_SPECIFIC_EVENT = Method(f"start-Flow1-{SECOND_ROUTINE_NAME}")
